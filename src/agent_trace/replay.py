...
# Short human-readable type labels (replaces truncated SCREAMING_SNAKE_CASE)
_TYPE_LABELS = {
    "session_start":      "START",
    "session_end":        "END",
    "user_prompt":        "USER",
    "assistant_response": "AGENT",
    "tool_call":          "TOOL",
    "tool_result":        "RESULT",
    "llm_request":        "LLM →",
    "llm_response":       "← LLM",
...
def _build_summary(event: TraceEvent, prev_events: list[TraceEvent]) -> tuple[str, bool]:
    ...
    if et == "tool_result":
        ...
        output = data.get("result") or data.get("output") or data.get("content_preview") or data.get("content") or ""
        # Handle Claude Code content blocks: [{"type": "text", "text": "..."}]
        if isinstance(output, list):
            output = " ".join(item.get("text", "") for item in output if isinstance(item, dict))
        snippet = _strip_markdown(str(output))[:50].replace("\n", " ").strip()
        if is_err:
...
def format_event(
    ...
    type_label = _TYPE_LABELS.get(et, et.upper()[:8])
    ...
    return f"{color_code}{icon} {rel:>7}  {type_label:<6}  {summary}{reset}"
